var PRODUCTS_LIST = [
  {
    id: 1,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 1",
    price: 135,
    images: {
      preview: "../assets/images/women-product-1.png",
      other: [
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-3.png",
        "../assets/images/women-product-4.png",
        "../assets/images/women-product-5.png",
        "../assets/images/women-product-6.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps.  ",
  },
  {
    id: 2,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 2",
    price: 125,
    images: {
      preview: "../assets/images/women-product-2.png",
      other: [
        "../assets/images/women-product-3.png",
        "../assets/images/women-product-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-5.png",
        "../assets/images/women-product-6.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 3,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 3",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 4,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 4",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 5,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 5",
    price: 115,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 6,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 6",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 7,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 7",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 8,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 8",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 9,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 9",
    price: 145,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
  {
    id: 10,
    title: "RECYCLED POLYESTER PERFORMANCE PARKA 10",
    price: 165,
    images: {
      preview: "../assets/images/women-fashion-1.png",
      other: [
        "../assets/images/women-fashion-2.png",
        "../assets/images/women-fashion-3.png",
        "../assets/images/women-fashion-4.png",
        "../assets/images/women-product-2.png",
        "../assets/images/women-product-1.png",
      ],
    },
    description:
      "Crafted from an organic cotton-mulberry silk mix, this oversized cardigan features a elasticated body and dropped shoulder. Effortlessly style with tonal shorts and pumps. - Relaxed fit - Dropped shoulder - Elasticated body - Ribbed cuff 32% Polyamide, 29% Viscose, 23% Mulberry silk, 15% Organic cotton, 1% Elastane / Machine washable Back length of size XS/S is 70cm ",
  },
];
